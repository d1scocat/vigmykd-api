...
async def register(
    ...
    logger.info(f"▶️ REGATT {log_id} continues to confirm stage: successfully sent code email")
...
async def confirm(
    ...
    if not ip:
        logger.warning(f"❌ CONFATT {log_id} failed: no IP available")
        return err(400, "Could not fetch your IP address")

    # ratelimit impl
    lockout_key = f"lockout:ip:{ip}:confirm"
    fail_key = f"failcount:ip:{ip}:confirm"

    if await redis.exists(lockout_key):
        await run_in_threadpool(argon.hash, "dummy")
        logger.error(f"❌ CONFATT {log_id}: rate-limited for {ip}")
    ...
    async def fail(status: int, msg: str):
        ...
        if fail_count == 1:
            logger.info(f"📊 CONFATT {log_id}: Failure count for {ip} = "
        ...
        if fail_count >= config.FAIL_COUNT_TO_CONFIRM_RATELIMIT:
            ...
            logger.error(f"🔒 CONFATT {log_id}: rate-locked {ip} "
                         f"after {config.FAIL_COUNT_TO_CONFIRM_RATELIMIT} fails")
            return err(429, "Too many requests, try again later")

        return err(status, msg)

    value = await redis.getdel(f"confirm:{query.code}")
    if value is None:
        logger.info(f"⚠️ CONFATT {log_id}: {query.code=} not found (expired/invalid/used)")
        return await fail(401, "The code is not valid.")

    try:
        res = json.loads(value)
    except json.JSONDecodeError:
        logger.warning(f"❌ CONFATT {log_id} failed: Corrupted payload for {query.code=}: {value}")
        return await fail(500, "The code has been corrupted. Please restart the registration process")

    if not {"type", "email", "pwhash", "nickname"}.issubset(res.keys()):
        logger.warning(f"❌ CONFATT {log_id} failed: {query.code=} was malformed: {res}")
        return await fail(500, "The code has been corrupted. Please restart the registration process")
    ...
    if conflict:
        if conflict["nickname"] == nickname:
            logger.warning(f"❌ CONFATT {log_id} failed: {nickname=} is in use")
            return await fail(409, f"Nickname {nickname} is already in use")
        else:
            logger.warning(f"❌ CONFATT {log_id} failed: {email=} is in use")
    ...
    try:
        ...
        logger.warning(f"❌ CONFATT {log_id} failed: {email=} or {nickname=} already taken")
        return await fail(409, f"Registration failed. Please try again")

    await ensure_constant_time()

    logger.info(f"▶️ CONFATT {log_id} successful: registered {email=}|{nickname=}|{ip=}")
